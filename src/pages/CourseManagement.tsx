...
export default function CourseManagement() {
  ...
  const [newCourse, setNewCourse] = useState({ title: '', description: '', thumbnail_url: '', category: '', is_free: false });
  ...
  const [instructorProfile, setInstructorProfile] = useState({ bio: '', avatar_url: '' });
  ...
  async function handleUpdateProfile() {
    ...
    if (error) alert('更新失敗：' + error.message + '\n請確認資料庫 profiles 表是否有 bio 與 avatar_url 欄位。');
    else alert('個人檔案更新成功！');
  ...
  async function handleSaveCourse() {
    if (editingCourse) {
      ...
      if (error) alert(error.message);
        ...
        setShowCourseModal(false);
        setNewCourse({ title: '', description: '', thumbnail_url: '', category: '' });
        fetchCourses();
      }
    }
  }
  ...
  async function handleAddUnit() {
    const { error } = await supabase.from('units').insert([{
      ...newUnit,
      course_id: selectedCourseForUnits.id
    }]);
    if (error) alert(error.message);
    else {
  ...
  async function handleUpdateQuiz() {
    const { error } = await supabase
      .from('units')
      .update({ quiz_data: editingQuizUnit.quiz_data })
      .eq('id', editingQuizUnit.id);
    ...
    <div className="max-w-7xl mx-auto px-4 py-32 space-y-12">
      <div className="flex justify-between items-end">
        ...
        <div className="flex space-x-4">
          <button 
            onClick={() => {
              setEditingCourse(null);
              setNewCourse({ title: '', description: '', thumbnail_url: '', category: '', is_free: false });
              setShowCourseModal(true);
            }}
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          >
            <div className="relative aspect-video">
              ...
              <div className="absolute top-4 right-4">
                <span className={cn(
                  ...
                  course.is_published ? "bg-green-500/20 text-green-400" : "bg-white/10 text-white/40"
                )}>
            ...
            <div className="p-6 flex-1 flex flex-col">
              ...
              <p className="text-white/40 text-sm line-clamp-2 mb-6">{course.description}</p>
          ...
          <section className="glass rounded-3xl p-8 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  type="text" 
                  value={instructorProfile.avatar_url} 
                  onChange={e => setInstructorProfile({...instructorProfile, avatar_url: e.target.value})}
              ...
              <div>
                ...
                <div className="quill-dark">
                  <ReactQuill
                    ...
                    onChange={val => setInstructorProfile({...instructorProfile, bio: val})}
      ...
      {showCourseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-dark/80 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass p-8 rounded-3xl w-full max-w-xl space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  type="text" value={newCourse.title} onChange={e => setNewCourse({...newCourse, title: e.target.value})}
              ...
              <div>
                ...
                <input 
                  type="text" value={newCourse.thumbnail_url} onChange={e => setNewCourse({...newCourse, thumbnail_url: e.target.value})}
                  className="w-full bg-white/5 border border-white/10 rounded-xl p-3 focus:border-gold outline-none"
                />
              </div>
              <div className="flex items-center space-x-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                ...
                />
                <label htmlFor="is_free" className="text-sm font-bold text-white cursor-pointer">設定為免費課程</label>
              </div>
            </div>
            <div className="flex space-x-4">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass p-8 rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto space-y-8 custom-scrollbar">
            ...
            <div className="p-6 bg-white/5 rounded-2xl border border-white/5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input 
                  type="text" placeholder="單元名稱" value={newUnit.title} onChange={e => setNewUnit({...newUnit, title: e.target.value})}
                  className="md:col-span-2 bg-white/5 border border-white/10 rounded-xl p-3 focus:border-gold outline-none text-sm"
                />
                <input 
                  type="number" placeholder="排序" value={newUnit.order_index} onChange={e => setNewUnit({...newUnit, order_index: parseInt(e.target.value)})}
                  className="bg-white/5 border border-white/10 rounded-xl p-3 focus:border-gold outline-none text-sm"
                />
                <input 
                  type="text" placeholder="YouTube 影片 ID (例如: dQw4w9WgXcQ)" value={newUnit.youtube_id} onChange={e => setNewUnit({...newUnit, youtube_id: e.target.value})}
                  className="md:col-span-3 bg-white/5 border border-white/10 rounded-xl p-3 focus:border-gold outline-none text-sm"
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              {units.length === 0 ? (
                <p className="text-center py-8 text-white/20">尚未建立任何單元</p>
              ) : (
                  ...
                  {units.map((unit, idx) => (
                    <div key={unit.id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
                      ...
                      <div className="flex items-center space-x-2">
                        <button 
                          onClick={() => {
                            setEditingQuizUnit(unit);
                            setShowQuizEditor(true);
                          }}
                          className="p-2 rounded-lg bg-gold/10 text-gold hover:bg-gold hover:text-navy-dark transition-all text-xs font-bold"
                        >
                          編輯測驗 ({unit.quiz_data?.length || 0})
                        </button>
                        <button onClick={() => handleDeleteUnit(unit.id)} className="text-red-400 hover:text-red-300 p-2">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              {editingQuizUnit.quiz_data?.map((q: any, qIdx: number) => (
                <div key={qIdx} className="p-6 bg-white/5 rounded-2xl border border-white/5 space-y-4 relative">
                  <button 
                    onClick={() => {
                      const newData = [...editingQuizUnit.quiz_data];
                      newData.splice(qIdx, 1);
                      setEditingQuizUnit({...editingQuizUnit, quiz_data: newData});
                    }}
                    ...
                    {q.options.map((opt: string, oIdx: number) => (
                      <div key={oIdx} className="flex items-center space-x-2">
                        ...
                        <input 
                          ...
                          className="flex-1 bg-white/5 border border-white/10 rounded-lg p-2 focus:border-gold outline-none text-xs"
              ...
              <button 
                onClick={() => {
                  const newData = editingQuizUnit.quiz_data ? [...editingQuizUnit.quiz_data] : [];
                  newData.push({ question: '', options: ['', '', '', ''], answer: 0 });
                  setEditingQuizUnit({...editingQuizUnit, quiz_data: newData});
                }}
